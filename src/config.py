...
def replace_with_env_variables(props, prefix=""):
    """
    ...
    :param prefix: parent property keys
    ...
    for key in props.keys():
        ...
        if type(props[key]) is dict:
            replace_with_env_variables(props[key], prefixed_key)
        else:
...
def get_properties(yaml_file):
    ...
    replace_with_env_variables(config)
    return config
